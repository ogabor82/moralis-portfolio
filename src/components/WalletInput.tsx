...
interface WalletInputProps {
  onAnalyze: (address: string, chain?: "eth" | "sol") => void;
  loading: boolean;
}
...
export const WalletInput: React.FC<WalletInputProps> = ({
  ...
  const validateEth = (addr: string): boolean =>
    /^0x[a-fA-F0-9]{40}$/.test(addr);
  const validateSol = (addr: string): boolean =>
    addr.length >= 32 && addr.length <= 44; // basic length check; Moralis validates server-side
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const valid = chain === "eth" ? validateEth(address) : validateSol(address);
    if (!valid) {
      setError(
        chain === "eth"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                value={chain}
                onChange={(e) => setChain(e.target.value as "eth" | "sol")}
